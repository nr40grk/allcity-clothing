...
export default function AdminProducts() {
  ...
  async function handleSave(e) {
    ...
    if (editingId) { setProducts(prev => prev.map(p => p.id === editingId ? { ...payload } : p)); flash('Product updated.'); }
    else { setProducts(prev => [...prev, { ...(saved.id ? saved : { ...payload, id: Date.now().toString() }), available: bool(payload.available), isNew: bool(payload.isNew) }]); flash('Product created.'); }
    resetForm();
  }

  async function handleDelete(id, name) {
    if (!confirm(`Delete "${name}"?`)) return;
    const res = await fetch('/api/admin', { method:'DELETE', headers:{ 'Content-Type':'application/json', 'x-admin-token': token }, body: JSON.stringify({ id }) });
      ...
      <div className="max-w-[1000px] mx-auto">
        <div className="flex flex-col gap-4 mb-10">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/admin/stock" className="font-mono text-xs uppercase tracking-widest border border-[#333] text-[#F0EDE8]/50 px-3 py-2 hover:border-[#FF2200] hover:text-[#FF2200] transition-colors">Stock</Link>
            ...
            <Link href="/admin/reviews" className="font-mono text-xs uppercase tracking-widest border border-[#333] text-[#F0EDE8]/50 px-3 py-2 hover:border-[#FF2200] hover:text-[#FF2200] transition-colors">Reviews</Link>
            ...
            <div>
              ...
              <div onDrop={handleDrop} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onClick={() => !uploading && fileInputRef.current?.click()}
                ...
                {uploading ? (
                  ...
                  <>
                    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-[#F0EDE8]/20"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {SIZES_OPTIONS.map(size => (
                  <button key={size} type="button" onClick={() => toggleSize(size)}
                    ...
                    className="font-mono text-xs px-3 py-1 border transition-colors">{size}</button>
                ))}
              </div>
            </div>
        ...
        {loading ? <p className="font-mono text-xs text-[#F0EDE8]/30">Loading...</p>
        : products.length === 0 ? <p className="font-mono text-xs text-[#F0EDE8]/20">No products yet. Click "+ New Product" to add one.</p>
        : (
