      ...
      <nav className="border-b border-[#1a1a1a] bg-[#080808]/95 backdrop-blur-sm">
        <div className="max-w-[1400px] mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="md:hidden text-[#F0EDE8]/60 hover:text-[#F0EDE8]" onClick={() => setMenuOpen(!menuOpen)}>
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#080808] border-t border-[#1a1a1a] px-6 py-6 flex flex-col gap-6">
            {[['/', 'nav.home'], ['/products', 'nav.products'], ['/shipping', 'nav.shipping']].map(([href, key]) => (
              <Link key={href} href={href} className="font-mono text-sm uppercase tracking-widest text-[#F0EDE8]/70 hover:text-[#FF2200] transition-colors" onClick={() => setMenuOpen(false)}>
                {t(key)}
              </Link>
            ))}
