      ...
      <div className="max-w-[1400px] mx-auto px-6 py-12 grid md:grid-cols-2 gap-10 lg:gap-20">
        <div className="flex flex-col gap-3">
          <div className="bg-[#111] aspect-[3/4] overflow-hidden">
            {images[activeImg] ? <img src={images[activeImg]} alt={product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><span className="font-mono text-xs text-[#F0EDE8]/20">No Image</span></div>}
          </div>
          {images.length > 1 && (
            <div className="flex gap-2">
              {images.map((src, i) => (
                <button key={i} onClick={() => setActiveImg(i)} className={`w-16 h-20 bg-[#111] overflow-hidden border transition-colors ${activeImg === i ? 'border-[#FF2200]' : 'border-transparent'}`}>
        ...
        <div className="flex flex-col gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              {onSale && <span className="font-mono text-[9px] uppercase tracking-widest bg-[#FF2200] text-[#080808] px-2 py-0.5">SALE</span>}
            </div>
            <h1 className="font-display text-4xl md:text-5xl text-[#F0EDE8] tracking-tight leading-tight">{product.name}</h1>
            {onSale ? (
              <div className="flex items-center gap-3 mt-2">
                <span className="font-mono text-xl text-[#FF2200]">€{Number(product.salePrice).toFixed(2)}</span>
                <span className="font-mono text-base text-[#F0EDE8]/30 line-through">€{Number(product.price).toFixed(2)}</span>
                <span className="font-mono text-[10px] bg-[#FF2200] text-[#080808] px-2 py-0.5">-{Math.round((1 - product.salePrice / product.price) * 100)}%</span>
              </div>
            ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {(product.sizes || []).map(size => (
                <button key={size} onClick={() => setSelectedSize(size)}
                  style={{ background: selectedSize === size ? '#FF2200' : 'transparent', borderColor: selectedSize === size ? '#FF2200' : '#333', color: selectedSize === size ? '#080808' : 'rgba(240,237,232,0.6)' }}
          ...
          {product.available ? (
            ...
            <div className="w-full font-mono text-xs uppercase tracking-widest border border-[#333] text-[#F0EDE8]/20 py-4 text-center cursor-not-allowed">{t('product.soldOut')}</div>
          )}
          ...
          <div className="border-t border-[#1a1a1a] pt-6">
            ...
            <ul className="flex flex-col gap-2">
              {(product.details || []).map(d => (
                <li key={d} className="font-mono text-xs text-[#F0EDE8]/40 flex items-center gap-2"><span className="text-[#FF2200]">—</span> {d}</li>
              ))}
            </ul>
          </div>
        </div>
