...
// Inlined translations — no external JSON imports that can break
const translations = {
  en: {
    ...
    home: {
      ...
      strip1Label: 'Worldwide Shipping', strip1Desc: 'Greece, Cyprus & Bulgaria: 1–3 days. International: calculated at checkout.',
    ...
    shipping: {
      ...
      returnsNote: '* If your order arrives damaged or not as described, contact us immediately. We will cover all additional fees.',
  ...
  el: {
    ...
    home: {
      ...
      strip1Label: 'Παγκόσμια Αποστολή', strip1Desc: 'Ελλάδα, Κύπρος & Βουλγαρία: 1–3 μέρες. Διεθνώς: υπολογίζεται στο checkout.',
      strip2Label: 'Street Ποιότητα', strip2Desc: 'Κάθε κομμάτι φτιαγμένο για την πόλη. Χωρίς συμβιβασμούς στα υλικά.',
    ...
    shipping: {
      ...
      shippingBody: 'Όλες οι παραγγελίες αποστέλλονται μέσω courier εντός 2 ημερών από την επιβεβαίωση.',
      ...
      note: '* Για οποιοδήποτε πρόβλημα σχετικά με την παραγγελία σου, επικοινώνησε μαζί μας.',
    ...
    checkout: {
      ...
      stripeNote: 'Ασφαλής πληρωμή μέσω Stripe. Δεν αποθηκεύουμε στοιχεία κάρτας.', pay: 'Πληρωμή', processing: 'Επεξεργασία...',
...
export function LanguageProvider({ children }) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('allcity_lang');
  ...
  function setLang(l) {
    setLangState(l);
    try { localStorage.setItem('allcity_lang', l); } catch {}
  }
